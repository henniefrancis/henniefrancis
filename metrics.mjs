...
//
// Run "node metrics.mjs --demo" to render metrics.demo.svg with sample data
// (no API calls) for local previewing.
...
// --- Profile + owned-repo stars + repos-contributed-to --------------------
...
function computeStreaks(days) {
  ...
  if ((days.get(today) || 0) === 0) cur.setUTCDate(cur.getUTCDate() - 1); // today not over yet
  let current = 0;
...
function buildSVG(stats) {
  ...
  // Theme - green / magenta terminal palette
  const title = "#00FF00";
  const value = "#FF00FF";
  const label = "#00FF00";
  const ic = "#00FF00";
...
(async () => {
  ...
  if (DEMO) {
    await writeFile("metrics.demo.svg", buildSVG(DEMO_STATS), "utf8");
    console.log("Wrote metrics.demo.svg (demo mode)");
    return;
  }
  const missing = [];
